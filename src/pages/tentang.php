<?php

use app\Config;

$config = new Config();

?>

<!-- Start banner_about -->
<section class="pt_banner_inner banner_px_image" id="Discover">
  <div class="parallax_cover">
    <img class="cover-parallax" src="<?= $config->assets ?>/img/inner/about.jpg" alt="image">
  </div>
  <div class="container">
    <div class="row">
      <div class="col-md-8 col-lg-6">
        <div class="banner_title_inner c-white">
          <h1 data-aos="fade-up" data-aos-delay="0">
            Tentang Kami
          </h1>
          <p data-aos="fade-up" data-aos-delay="100" class="banner_title_inner c-yollow">
            KADEr KEREN - SMP Negeri 5 Trenggalek
          </p>
        </div>
      </div>

    </div>
  </div>
</section>
<!-- End banner_about -->

<!-- Start about_cc_grid -->
<section class="about_cc_grid padding-py-12" id="About">
  <div class="container">
    <div class="row">
      <div class="col-lg-5">
        <div class="title_sections_inner mb-0">
          <div class="before_title">
            <span class="c-orange-red">Apa itu KADEr KEREN ? </span>
          </div>
          <h2>Tekad Bermasa Depan Cerah bagi Siswa Kelompok Rentan oleh SMP Negeri 5 Trenggalek</h2>
        </div>
      </div>
      <div class="col-lg-6 ml-auto">
        <div class="title_sections_inner mb-0">
          <p class="c-blue font-weight-bold">KADER KEREN adalah program inklusi dengan support aplikasi marketing berbasis web yang memprioritaskan peningkatan karakter positif dan kecerdasan finansial bagi anak-anak marjinal dan rentan putus sekolah level SMP. Program yang mempunyai impak afirmatif dan prospek keberlangsungan jangka panjang ini mempunyai beberapa strategi distingtif sebagai berikut:</p>
          <ol>
            <li class="c-blue"><strong>Pendampingan orang tua asuh untuk pengembangan karakter positif.</strong><br>
              <p class="c-blue">Orang tua asuh merupakan pendamping, motivator, role model dan mentor siswa, yang berasal dari sukarelawan guru, wali murid, dan komite sekolah. Pendampingan orang tua asuh dilakukan seminggu sekali secara intensif, dengan mengkolaborasikan kegiatan coaching religi, literasi, dan kegiatan sosial lainnya.</p>
            </li>
            <li class="c-blue"><strong>Program kewirausahaan untuk pengembangan kecerdasan finansial.</strong><br>
              <ol>
                <li class="c-blue">Tahap coaching melalui pemberian motivasi kewirausahaan dengan menghadirkan narasumber dari 6 IDUKA (Industri Dunia Usaha dan Dunia Kerja). Keenam IDUKA tersebut yakni HIJAB (Himpunan Janda Berkarya), Warung Bu Sihir, A &amp; A Tailor, Zia Handmade, Kriya Kayu Wonoanti, dan pengrajin logam Bintang Sejati.</li>
                <li class="c-blue">Tahap training kewirausahaan dan pembuatan produk dari 4 keterampilan (keterampilan dasar otomotif/logam, bangunan/kriya kayu, tata busana, dan tata boga). Keterampilan ini merupakan ekstrakurikuler wajib sehingga setiap siswa wajib mengikuti salah satu. Untuk mempertajam skill dan keterampilan anak asuh, maka diadakan pelatihan selama 10 kali pertemuan dengan tenaga ahli dari IDUKA.</li>
                <li class="c-blue">Tahap Pemasaran online melalui aplikasi pemasaran digital berbasis web dan offline dengan menitipkan produk di tempat usaha IDUKA. Bersama orang tua asuh, setiap anak asuh memasarkan produk tersebut dan bekerja keras untuk mencapai target profit penjualan.&nbsp;</li>
              </ol>
            </li>
          </ol>
        </div>
      </div>
    </div>

    <div class="row justify-content-center margin-t-12" data-aos="fade-up" data-aos-delay="0">
      <div class="col-lg-5 text-right">
        <div class="group_img">
          <img src="<?= $config->assets ?>/img/inner/a01.png" alt="">
          <img src="<?= $config->assets ?>/img/inner/a03.png" alt="">
        </div>
      </div>
      <div class="col-lg-7 text-left">
        <div class="group_img_two">
          <img src="<?= $config->assets ?>/img/inner/a02.png" alt="">
          <img src="<?= $config->assets ?>/img/inner/a04.png" alt="">
        </div>
      </div>
    </div>
  </div>
</section>
<!-- End. about_cc_grid -->

<section class="faq_section padding-py-6 mt-0 w-100">
  <div class="container">

    <div class="title_sections_inner text-center mt-2">
      <h2 class="font-weight-bold">STRUKTUR INOVASI "KADER KEREN"</h2>
      <h5>TAHUN 2023</h5>
    </div>
    <img src="<?= $config->assets ?>/refs/struktur-org.png" alt="" class="w-100">

  </div>
</section>

<section class="service__workspace padding-py-6 mt-0 w-100">
  <div class="container">

    <div class="table-responsive">
      <table class="table table-bordered table-hover align-middle nip-fmt">
        <thead>
          <tr class="font-weight-bold">
            <td>NO</td>
            <td>NAMA GURU</td>
            <td>PANGKAT/GOL</td>
            <td>JABATAN DALAM POKJA</td>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td rowspan="2">1</td>
            <td>LILIS RATNAWATI, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Penanggung Jawab</td>
          </tr>
          <tr>
            <td>NIP. 196812011997022002</td>
          </tr>
          <tr>
            <td rowspan="2">2</td>
            <td>PRIYO SUROSO, M.Pd.</td>
            <td rowspan="2">Penata Tk 1/IIId</td>
            <td rowspan="2">Pendamping</td>
          </tr>
          <tr>
            <td>NIP. 197403202006041014</td>
          </tr>
          <tr>
            <td rowspan="2">3</td>
            <td>SISWOYO, S.Pd.</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Ketua</td>
          </tr>
          <tr>
            <td>NIP. 196501131994121002</td>
          </tr>
          <tr>
            <td rowspan="2">4</td>
            <td>MISTRI UTAMI, S.Pd.</td>
            <td rowspan="2">Penata/IIIc</td>
            <td rowspan="2">Sekretaris</td>
          </tr>
          <tr>
            <td>NIP. 198411292009032012</td>
          </tr>
          <tr>
            <td rowspan="2">5</td>
            <td>KATMIATI</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Bendahara</td>
          </tr>
          <tr>
            <td>NIP. 196504181991032009</td>
          </tr>
          <tr>
            <td colspan="4" class="h4 font-weight-bold py-4">A. POKJA PRODUKSI</td>
          </tr>
        </tbody>
        <thead>
          <tr class="font-weight-bold">
            <td>NO</td>
            <td>NAMA GURU</td>
            <td>PANGKAT/GOL</td>
            <td>JABATAN DALAM POKJA</td>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td rowspan="2">1</td>
            <td>Drs. HERY SUSENO</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Kordinator</td>
          </tr>
          <tr>
            <td>NIP. 196212201993031002</td>
          </tr>
          <tr>
            <td rowspan="2">2</td>
            <td>Dra. SUCIATI</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 19630112 199512 2 001</td>
          </tr>
          <tr>
            <td rowspan="2">3</td>
            <td>CIPTO UTOMO, S.Pd.</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 196701311995121002</td>
          </tr>
          <tr>
            <td rowspan="2">4</td>
            <td>KATMIATI</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 196504181991032009</td>
          </tr>
          <tr>
            <td colspan="4" class="h4 font-weight-bold py-4">B. POKJA SIRKULASI PRODUKSI</td>
          </tr>
        </tbody>
        <thead>
          <tr class="font-weight-bold">
            <td>NO</td>
            <td>NAMA GURU</td>
            <td>PANGKAT/GOL</td>
            <td>JABATAN DALAM POKJA</td>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td rowspan="2">1</td>
            <td>HAJAR ROHMAH, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Kordinator</td>
          </tr>
          <tr>
            <td>NIP. 197305181997032004</td>
          </tr>
          <tr>
            <td rowspan="2">2</td>
            <td>RAHMAT TRI SANJAYA, S.Pd.</td>
            <td rowspan="2">-</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>-</td>
          </tr>
          <tr>
            <td colspan="4" class="h4 font-weight-bold py-4">C. POKJA ORANG TUA ASUH</td>
          </tr>
        </tbody>
        <thead>
          <tr class="font-weight-bold">
            <td>NO</td>
            <td>NAMA GURU</td>
            <td>PANGKAT/GOL</td>
            <td>JABATAN DALAM POKJA</td>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td rowspan="2">1</td>
            <td>Dra. HENI TRISISWATI</td>
            <td rowspan="2">Pembina Utama Muda/IVc</td>
            <td rowspan="2">Kordinator</td>
          </tr>
          <tr>
            <td>NIP. 196303031988032012</td>
          </tr>
          <tr>
            <td rowspan="2">2</td>
            <td>TUTIK PURNOMO RESTYANI, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>197012221997032000</td>
          </tr>
          <tr>
            <td rowspan="2">3</td>
            <td>MARZUKI, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 196602081989031010</td>
          </tr>
          <tr>
            <td rowspan="2">4</td>
            <td>DYAH KUSUMANINGTYAS, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 196507281989022003</td>
          </tr>
          <tr>
            <td rowspan="2">5</td>
            <td>TRI WINARSIH, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 197104221997032005</td>
          </tr>
          <tr>
            <td rowspan="2">6</td>
            <td>SLAMET HARIADI, S.Pd.</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 197111221997031004</td>
          </tr>
          <tr>
            <td rowspan="2">7</td>
            <td>Drs. SISWANTO, M.M.</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 196209211991031006</td>
          </tr>
          <tr>
            <td rowspan="2">8</td>
            <td>DIYAH PURNAMANINGRUM, S.Pd. M.Pd.</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 197410292002122004</td>
          </tr>
          <tr>
            <td rowspan="2">9</td>
            <td>KATMIATI</td>
            <td rowspan="2">Pembina/IVa</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 196504181991032009</td>
          </tr>
          <tr>
            <td rowspan="2">10</td>
            <td>MISTRI UTAMI, S.Pd.</td>
            <td rowspan="2">Penata/IIIc</td>
            <td rowspan="2">Anggota</td>
          </tr>
          <tr>
            <td>NIP. 198411292009032012</td>
          </tr>
          <tr>
            <td rowspan="2">11</td>
            <td>SITI KOMARIYAH, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota/Guru</td>
          </tr>
          <tr>
            <td>NIP. 197105271997032008</td>
          </tr>
          <tr>
            <td rowspan="2">12</td>
            <td>DEWI MUSLIHAH, S.Pd.</td>
            <td rowspan="2">Penata/IIIc</td>
            <td rowspan="2">Anggota/Guru</td>
          </tr>
          <tr>
            <td>NIP. 197909042006042020</td>
          </tr>
          <tr>
            <td rowspan="2">13</td>
            <td>AGUS ENDANG WIDAYATI, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota/Guru</td>
          </tr>
          <tr>
            <td>NIP. 196608261989032003</td>
          </tr>
          <tr>
            <td rowspan="2">14</td>
            <td>ANGGRAINI, S.Pd.</td>
            <td rowspan="2">Pembina Tk 1/IVb</td>
            <td rowspan="2">Anggota/Guru</td>
          </tr>
          <tr>
            <td>NIP. 197003061998032005</td>
          </tr>
          <tr>
            <td rowspan="2">15</td>
            <td>ERNA ZULAIKHA, S.Pd</td>
            <td rowspan="2">Penata Tk 1/IIId</td>
            <td rowspan="2">Anggota/Guru</td>
          </tr>
          <tr>
            <td>NIP. 197002172007012020</td>
          </tr>
          <tr>
            <td rowspan="2">16</td>
            <td>SITI ROHMAH, S.Pd.</td>
            <td rowspan="2">Penata Tk 1/IIId</td>
            <td rowspan="2">Anggota/Guru</td>
          </tr>
          <tr>
            <td>NIP. 196902122000122003</td>
          </tr>
          <tr>
            <td>17</td>
            <td>RINTA MARA ALPHADORA</td>
            <td>-</td>
            <td>Anggota/Wali Murid</td>
          </tr>
          <tr>
            <td>18</td>
            <td>ARI HERNAWAN</td>
            <td>-</td>
            <td>Anggota/Wali Murid</td>
          </tr>
          <tr>
            <td>19</td>
            <td>SUNARDIANTO</td>
            <td>-</td>
            <td>Anggota/Wali Murid</td>
          </tr>
          <tr>
            <td>20</td>
            <td>SUTARJI</td>
            <td>-</td>
            <td>Anggota/Komite</td>
          </tr>
          <tr>
            <td>21</td>
            <td>YUDAN YUNIARTO</td>
            <td>-</td>
            <td>Anggota/Komite</td>
          </tr>
          <tr>
          </tr>
        </tbody>
      </table>
    </div>

  </div>
</section>

<section class="service__workspace features__workspace padding-py-6 mt-0 w-100">
  <div class="container">

    <div class="title_sections_inner text-center mt-2">
      <h4 class="font-weight-bold">DAFTAR ANAK ASUH (BUTUH PENANGANAN INTENSIF)</h4>
      <h6>TAHUN 2023</h6>
    </div>

    <div class="table-responsive">
      <table class="table table-bordered table-hover table-striped">
        <thead>
          <tr class="font-weight-bold">
            <td>NO INDUK</td>
            <td>NISN</td>
            <td>NAMA SISWA</td>
            <td>KELAS</td>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>007090</td>
            <td>3084312133</td>
            <td>WAHYU NUR NILAMSARI</td>
            <td>VIII A</td>
          </tr>
          <tr>
            <td>007093</td>
            <td>0095452459</td>
            <td>ABID RIJAL KAMAL</td>
            <td>VII IB</td>
          </tr>
          <tr>
            <td>007145</td>
            <td>3094642008</td>
            <td>MOHAMMAD AN'IM FALAHUDIN</td>
            <td>VIII C</td>
          </tr>
          <tr>
            <td>007139</td>
            <td>0083247614</td>
            <td>GEGER SATRIO</td>
            <td>VIII C</td>
          </tr>
          <tr>
            <td>007177</td>
            <td>0084450587</td>
            <td>NILA AYU NOVITASARI</td>
            <td>VIII D</td>
          </tr>
          <tr>
            <td>007236</td>
            <td>0085427380</td>
            <td>RAISA</td>
            <td>VIII F</td>
          </tr>
          <tr>
            <td>007249</td>
            <td>0075033631</td>
            <td>DIKA ABDULAH</td>
            <td>VIII G</td>
          </tr>
          <tr>
            <td>006877</td>
            <td>0075334116</td>
            <td>GESANG BAYUADJI PUTRA. P</td>
            <td>IX B</td>
          </tr>
          <tr>
            <td>006923</td>
            <td>0078969093</td>
            <td>REVAL HUSSEN ASKARI</td>
            <td>IX C</td>
          </tr>
          <tr>
            <td>006992</td>
            <td>0079881504</td>
            <td>TONI TONATA ADRIANSYAH</td>
            <td>IX E</td>
          </tr>
          <tr>
            <td>007151</td>
            <td>0085936119</td>
            <td>ULFA NUR AGUSTIN</td>
            <td>VIII C</td>
          </tr>
          <tr>
            <td>007168</td>
            <td>3082214918</td>
            <td>JESICHA UCHIANA F</td>
            <td>VIII D</td>
          </tr>
          <tr>
            <td>007204</td>
            <td>0086326426</td>
            <td>MAYANG PUTRI AYU ANDIRA</td>
            <td>VIII E</td>
          </tr>
          <tr>
            <td>007219</td>
            <td>3082803829</td>
            <td>AURORA DEWI SETYASARI</td>
            <td>VIII F</td>
          </tr>
          <tr>
            <td>007261</td>
            <td>0093479041</td>
            <td>RETNO DYAN LESTARI</td>
            <td>VIII G</td>
          </tr>
          <tr>
            <td>007253</td>
            <td>0091210157</td>
            <td>HENI ARIFIANTI</td>
            <td>VIII G</td>
          </tr>
          <tr>
            <td>007066</td>
            <td>0083232613</td>
            <td>BAGAS BINTARA YUDHA</td>
            <td>VIII A</td>
          </tr>
          <tr>
            <td>007067</td>
            <td>0085212574</td>
            <td>BAYU CIPTA PRATAMA</td>
            <td>VIII A</td>
          </tr>
          <tr>
            <td>007142</td>
            <td>0082932590</td>
            <td>MARCEL AFFANDI FERNANDES</td>
            <td>VIII C</td>
          </tr>
          <tr>
            <td>007148</td>
            <td>0087525695</td>
            <td>RAZYA EKA SEPTIANDIKA</td>
            <td>VIII C</td>
          </tr>
          <tr>
            <td>007155</td>
            <td>0086007266</td>
            <td>ADJI SETIAWAN</td>
            <td>VIII D</td>
          </tr>
        </tbody>
      </table>
    </div>
  </div>
</section>